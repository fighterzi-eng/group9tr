...
class CyberTronRestaurant:
    ...
    def add_items_to_category(self, category, frame):
        ...
        for item, details in self.menu_items[category].items():
            ...
            name_label = tk.Label(item_frame, text=item, font=("Arial", 14, "bold"), fg="cyan", bg="#64646d")
            name_label.pack(pady=(10, 5))

            desc_label = tk.Label(item_frame, text=details["description"], font=("Arial", 10), fg="white", bg="#64646d", wraplength=200)
            desc_label.pack(pady=5)

            size_frame = tk.Frame(item_frame, bg="#64646d")
            size_frame.pack(pady=5)
            
            size_label = tk.Label(size_frame, text="Size:", font=("Arial", 10, "bold"), fg="white", bg="#64646d")
            size_label.pack(side="left")
            
            size_var = tk.StringVar(value="M")
            for size, price in details["sizes"].items():
                size_btn = tk.Radiobutton(size_frame, text=f"{size} ({price} EGP)", variable=size_var, value=size, fg="white", bg="#64646d", selectcolor="#4a4a8a", font=("Arial", 9))
            ...
            for addon, addon_price in details["addons"].items():
                var = tk.BooleanVar()
                chk = tk.Checkbutton(addon_frame, text=f"{addon} (+{addon_price} EGP)", variable=var, fg="white", bg="#64646d", selectcolor="#4a4a8a", font=("Arial", 9))
                chk.pack(anchor="w")
                addon_vars[addon] = (var, addon_price)

            add_btn = tk.Button(item_frame, text="Add to Cart", command=lambda i=item, s=size_var, a=addon_vars, c=category: self.add_to_cart(i, s, a, c), bg="#00cc66", fg="white", font=("Arial", 10, "bold"))
    ...
    def create_cart_frame(self):
        ...
        cart_title = tk.Label(self.cart_frame, text="Your Cart", font=("Arial", 16, "bold"), fg="cyan", bg="#0a0a1a")
        ...
        delivery_chk = tk.Checkbutton(self.cart_frame, text="Home Delivery (+10 EGP)", variable=self.delivery_var, command=self.update_total, fg="white", bg="#0a0a1a", selectcolor="#4a4a8a", font=("Arial", 12))
        delivery_chk.pack(anchor="w", pady=5)

        self.total_label = tk.Label(self.cart_frame, text="Total: 0 EGP", font=("Arial", 14, "bold"), fg="yellow", bg="#0a0a1a")
        self.total_label.pack(anchor="w", pady=5)

        btn_frame = tk.Frame(self.cart_frame, bg="#0a0a1a")
        btn_frame.pack(fill="x", pady=10)

        confirm_btn = tk.Button(btn_frame, text="Confirm Order", command=self.confirm_order, bg="#1eff00", fg="white", font=("Arial", 12, "bold"))
        confirm_btn.pack(side="left", padx=5)

        cancel_btn = tk.Button(btn_frame, text="Cancel Order", command=self.cancel_order, bg="#ff6666", fg="white", font=("Arial", 12, "bold"))
        cancel_btn.pack(side="left", padx=5)
        
        reset_btn = tk.Button(btn_frame, text="Reset", command=self.reset_cart, bg="#ffcc00", fg="black", font=("Arial", 12, "bold"))
    ...
    def update_cart_display(self):
        ...
        if not self.cart:
            empty_label = tk.Label(self.cart_items_frame, text="Your cart is empty", font=("Arial", 12), fg="white", bg="#0a0a1a")
        ...
        for i, item in enumerate(self.cart):
            ...
            item_label = tk.Label(self.cart_items_frame, text=item_text, font=("Arial", 10), fg="white", bg="#0a0a1a")
    ...
    def confirm_order(self):
        ...
        print("==========================\n")
        rospy.wait_for_service('requist')
        try:
            requist_service = rospy.ServiceProxy('requist', requist)
            order_id = str(order_number)
            items = [item['item'] for item in self.cart]
            sizes = [item['size'] for item in self.cart]
            extras = [', '.join(item['addons']) if item['addons'] else 'None' for item in self.cart]
            delivery = self.delivery_var.get()
            response = requist_service(order_id, items, sizes, extras, delivery)
            if response.success:
        ...
        confirm_msg = f"Confirm your order {delivery_type}?\n\n{order_details}\n\nTotal: {total} EGP"
