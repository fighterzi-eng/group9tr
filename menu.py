...
#             order_details += f"{item} x {qty} = ${menu[item] * int(qty)}\n"
#     order_details += f"\nGrand Total = ${total}"
...
# title_label = tk.Label(root, text="Welcome to the Energgon restaurant!", font=("Helvetica", 24, "bold"))
...
#     tk.Label(frame, text=f"{item} --> {price} EGP", font=("Helvetica", 12)).grid(row=row, column=0, padx=10, pady=5, sticky="w")
#     qty_var = tk.StringVar()
#     quantities[item] = qty_var
#     tk.Entry(frame, textvariable=qty_var, width=5).grid(row=row, column=1, padx=10)
#     row += 1
...
# cart_img = tk.PhotoImage(file="/home/kenzy/vsCode projects/cart.png")
# cart_label = tk.Label(root, image=cart_img)
# cart_label.image = cart_img
# cart_label.pack()



# tk.Button(btn_frame, text="Calculate Total", command=calculate_total, bg="lightblue").grid(row=0, column=0, padx=10)
# tk.Button(btn_frame, text="Confirm Order", command=confirm_order, bg="lightgreen").grid(row=0, column=1, padx=10)
# tk.Button(btn_frame, text="Reset", command=reset_order, bg="orange").grid(row=0, column=2, padx=10)
# tk.Button(btn_frame, text="Exit", command=root.quit, bg="red").grid(row=0, column=3, padx=10)

# # Total label
# total_label = tk.Label(root, text="Total: 0 EGP", font=("Helvetica", 14, "bold"))
